...
function sendState(payload = {}) {
  ...
  detailWindow.webContents.send('aftersale-detail-state', payload);
}
...
async function createAfterSaleDetailWindow(parent) {
  ...
  try {
    await detailWindow.loadFile(path.join(__dirname, '..', 'renderer', 'aftersale-detail-window.html'));
  } catch (err) {
    if (detailWindow && !detailWindow.isDestroyed()) {
...
function getAfterSaleDetailWindow() {
  return detailWindow;
}

function setAfterSaleDetailToolbarHeight(height) {
...
function loadAfterSaleDetailUrl(store, shopId, url) {
  if (!detailWindow || detailWindow.isDestroyed()) {
