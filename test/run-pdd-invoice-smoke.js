...
async function main() {
  ...
  const client = new InvoiceApiClient(SHOP_ID, {
    onLog(message) {
      console.log('[PDD INVOICE TEST]', message);
    },
    getShopInfo() {
      return {
        id: SHOP_ID,
    ...
    getInvoiceUrl() {
      return 'https://mms.pinduoduo.com/invoice/center?msfrom=mms_sidenav';
    },
  });
  ...
  try {
    ...
    result.steps.push({ step: 'getOverview', ok: true, pendingNum: result.overview.pendingNum });

    result.list = await client.getList({ pageNo: 1, pageSize: 10 });
    result.steps.push({ step: 'getList', ok: true, total: result.list.total, count: result.list.list.length });
